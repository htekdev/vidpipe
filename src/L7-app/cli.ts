...
program
  ...
  .option('--add', 'Add a single idea (manual creation)')
  .option('--topic <topic>', 'Idea topic/title (required with --add)')
  .option('--hook <hook>', 'Attention-grabbing hook (default: topic)')
  .option('--audience <audience>', 'Target audience (default: developers)')
  .option('--platforms <platforms>', 'Comma-separated platforms: tiktok,youtube,instagram,linkedin,x')
  .option('--key-takeaway <takeaway>', 'Core message the viewer should remember')
  .option('--talking-points <points>', 'Comma-separated talking points')
  .option('--tags <tags>', 'Comma-separated categorization tags')
  .option('--publish-by <date>', 'Publish deadline (ISO 8601 date, default: 14 days from now)')
  .option('--trend-context <context>', 'Why this topic is timely')
  .option('--prompt <prompt>', 'Additional guidance for AI enrichment')
  .option('--no-ai', 'Disable AI enrichment (use defaults for missing fields)')
  .action(async (opts) => {
    initConfig()
