...
For full examples (developer, corporate, educator templates) and additional tips, see the [Brand Customization Guide](/guide/brand-customization).
...
The `schedule.json` file defines when social media posts are published. It is generated automatically by the pipeline and can be managed via `vidpipe schedule`. For full details on scheduling and the review workflow, see the [Social Publishing Guide](/guide/social-publishing).
...
- Install FFmpeg (see [FFmpeg Setup](/guide/ffmpeg-setup))
